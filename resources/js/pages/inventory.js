...
const state = {
    ...
    function renderTable() {
        ...
        body.innerHTML = rows.map(p => {
            ...
            return `
                <tr>
                    <td>
                        <div class="prod-cell">${icon}<span>${p.name}</span></div>
                    </td>
                    <td class="sku-cell">${p.sku}</td>
                    <td><span class="cat-pill">${icon}${p.category}</span></td>
                    <td class="price-cell">${fmtMoney(p.price)}</td>
                    <td><span class="stock-pill ${sCls}">${p.stock}</span></td>
                    <td>
                        <div class="actions-cell">
                            <button class="btn-edit" data-sku="${p.sku}">EDIT</button>
                            <button class="btn-del" data-sku="${p.sku}">DEL</button>
                        </div>
                    </td>
                </tr>`;
        }).join('');
    ...
    function bindEvents() {
        ...
        document.getElementById('productBody').addEventListener('click', (e) => {
            ...
            if (editBtn) {
                // Hook up your edit-product modal here.
                console.log('Edit product', editBtn.dataset.sku);
            }
            if (delBtn) {
                // Hook up your delete confirmation / request here.
                console.log('Delete product', delBtn.dataset.sku);
            }
        });
